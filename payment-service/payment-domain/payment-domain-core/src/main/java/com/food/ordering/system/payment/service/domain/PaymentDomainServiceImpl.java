...
public class PaymentDomainServiceImpl implements PaymentDomainService {
    ...
    private void validateCreditHistory(CreditEntry creditEntry, List<CreditHistory> creditHistories, List<String> failureMessages) {
        ...
        if(totalDebitHistory.isGreaterThan(totalCreditHistory)){
            String message = "Customer with id=" + creditEntry.getCustomerId().getValue() + " doesn't gave enough credit according to credit history";
            log.error(message);
            failureMessages.add(message);
        }

        if(!creditEntry.getTotalCreditAmount().equals(totalCreditHistory.subtract(totalDebitHistory))){
            String message = "Credit history total is not equal to current credit for customer id = " + creditEntry.getCustomerId().getValue();
            log.error(message);
            failureMessages.add(message);
        }
    }
